...
class OrderController extends Controller
{
    ...
    {
        Admin::js('/vendor/laravel-admin/layer/2.4/layer.js');
...
//        $grid->column('user.name', '用户');
        ...
        $grid->column('status', '状态')->display(function ($status) {
            ...
            // 如果订单是付款, 那么就修改为物流状态
            if ($status == BaseConstants::ORDER_PAID) {
...
//
//            // 如果出现了申请,显示可以退款按钮
//            if ($order->status == BaseConstants::ORDER_APP_REFUND) {
//                // append一个操作
//                $actions->append("<a href='{$url}' title='退款'><i class='fa fa-mail-reply'></i></a>");
//            } elseif ($order->status == BaseConstants::ORDER_PAID) {
//
//                if ($order->ship_status == BaseConstants::LOGISTICS_PENDING) {
//
//                    $actions->append(new ShipButton($order->id));
//                } elseif ($order->ship_status == BaseConstants::LOGISTICS_DELIVERED) {
//
...
//        $show->divider();
        ...
        $show->field('consignee_name', '联系人');
        $show->field('consignee_phone', '联系人手机');
        $show->field('consignee_address', '地址');
...
//            $details->column('product_id', '名称');

            $details->column('product_id','名称')->display(function ($title, $column) {

                // 如果这一列的status字段的值等于1，直接显示title字段
                if ($this->product_id == 0) {
                    return '企业帮个环境消毒控菌（元/平米）';
                }
                if ($this->product_id == 1) {
                    return '企业办公环境除甲醛（元/平米）';
                }
                if ($this->product_id == 2) {
                    return '空气净化器租赁（元/台、每月）';
                }
                if ($this->product_id == 3) {
                    return '家庭室内除甲醛（元/平米）';
                }
                if ($this->product_id == 4) {
                    return '家庭室内消毒控菌（元/平米）';
                }
...
//            });
            $details->column('total', '小计')->totalRow(function ($amount) {

                return "<span class='text-danger text-bold'>合计：<i class='fa fa-yen'></i> {$amount} 元</span>";
    ...
    public function refund(Order $order)
    {
        ...
        try {
            ...
            $order->status = BaseConstants::ORDER_REFUND;
            $order->save();
    ...
    public function ship(Request $request, Order $order)
    {
        ...
        if ($order->status != BaseConstants::ORDER_PAID) {
        ...
        $order->ship_status = BaseConstants::LOGISTICS_DELIVERED;
    ...
    public function confirmShip(Order $order)
    {
        ...
        if ($order->ship_status != BaseConstants::LOGISTICS_DELIVERED) {

            return back()->withErrors('订单未发货', 'error');
        }

        $order->ship_status = BaseConstants::LOGISTICS_RECEIVED;
        $order->save();
